...
function findParent(id, items = [root]) {
  ...
  return (
    items.find((item) => item.children?.some((c) => c.id === id)) ??
    findParent(
      id,
...
function findItemBefore(item) {
  return findParent(item.id).children.find(
    (_, i, all) => all[i + 1]?.id === item.id
  );
}
...
function injectNextTo(item, target, after = true) {
  ...
  targetParent.children.splice(
    targetParent.children.indexOf(target) + (after ? 1 : 0),
    0,
    item
  );
...
export default {
  ...
  methods: {
    ...
    findItemBefore(item) {
      return this.findParent(item.id).children.find(
        (_, i, all) => all[i + 1]?.id === item.id
      );
    },
    findItemAfter(item) {
