Subheader
=========

.. aknowledgement::
    This page is a Python adaptation of the `official Vuetify Subheader
    documentation <https://v2.vuetifyjs.com/en/components/subheaders/>`__. All
    examples have been converted to pyvuetify syntax.

The :py:class:`Subheader <pyvuetify.Subheader>` component is used to separate
sections of lists.

.. api::

    - :py:class:`pyvuetify.Subheader`

Usage
-----

Subheaders in their simplest form display a subheading with default theme.

.. jupyter-execute:: Subheader/usage.py
    :raises:
    :hide-code:

Examples
--------

Inset
^^^^^

Inset subheaders are moved 72px to the right. This gives you the option to
line them up with list items and inset dividers.

.. tab-set::

    .. tab-item:: :fas:`eye` Rendered

        .. jupyter-execute:: Subheader/inset.py
            :raises:
            :hide-code:

    .. tab-item:: :fab:`python` Python

        .. literalinclude:: Subheader/inset.py

    .. tab-item:: :fab:`vuejs` Vue template

        .. literalinclude:: Subheader/inset.vue

Misc
----

Grid
^^^^

A subheader can add context to what a user is looking at.

.. tab-set::

    .. tab-item:: :fas:`eye` Rendered

        .. jupyter-execute:: Subheader/grid.py
            :raises:
            :hide-code:

    .. tab-item:: :fab:`python` Python

        .. literalinclude:: Subheader/grid.py

    .. tab-item:: :fab:`vuejs` Vue template

        .. literalinclude:: Subheader/grid.vue

Menu
^^^^

Using a subheader can help separate different types of actions.

.. tab-set::

    .. tab-item:: :fas:`eye` Rendered

        .. jupyter-execute:: Subheader/menu.py
            :raises:
            :hide-code:

    .. tab-item:: :fab:`python` Python

        .. literalinclude:: Subheader/menu.py

    .. tab-item:: :fab:`vuejs` Vue template

        .. literalinclude:: Subheader/menu.vue

Social media
^^^^^^^^^^^^

Using a subheader with social media interaction.

.. tab-set::

    .. tab-item:: :fas:`eye` Rendered

        .. jupyter-execute:: Subheader/social_media.py
            :raises:
            :hide-code:

    .. tab-item:: :fab:`python` Python

        .. literalinclude:: Subheader/social_media.py

    .. tab-item:: :fab:`vuejs` Vue template

        .. literalinclude:: Subheader/social_media.vue

