  ...
  <v-card flat>
    <v-card-text>
      <div class="d-flex justify-center align-center ga-4 mb-6">
        <v-btn-toggle v-model="variant" variant="outlined" divided mandatory>
        ...
        <v-checkbox v-model="icons" label="Icons" hide-details></v-checkbox>
      </div>
      ...
      <v-container fluid>
        <v-row
          ...
          :class="{ 'mt-12': i > 0 }"
        >
          <v-col v-for="(pair, j) in group" :key="j" cols="12" md="4" sm="4">
            <v-switch
              v-for="color in pair"
              :key="color"
              v-model="ex11"
