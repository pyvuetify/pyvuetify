...
The :py:class:`BtnToggle <pyvuetify.BtnToggle>` component is a simple wrapper for :py:class:`ItemGroup <pyvuetify.ItemGroup>` built specifically to work with :py:class:`Btn <pyvuetify.Btn>`.
...
A :py:class:`BtnToggle <pyvuetify.BtnToggle>` with the ``mandatory`` prop will always have a value.
...
A :py:class:`BtnToggle <pyvuetify.BtnToggle>` with the ``multiple`` prop will allow a user to select
...
You can make :py:class:`BtnToggle <pyvuetify.BtnToggle>` rounded using the ``rounded`` prop.
