...
function US(e, t) {
  ...
  const {
    ...
    Ee && Se.beforeEnter(le), a(le, $, j), ((ie = ge && ge.onVnodeMounted) || Ee || we) && Ht(() => {
      ie && Cn(ie, re, M), Ee && Se.enter(le), we && _a(M, null, re, "mounted");
    }, z);
  }, I = (M, $, j, re, z) => {
    if (j && v(M, j), re)
    ...
    if (Ko(M) && (Y.ctx.renderer = _e), lk(Y, !1, me), Y.asyncDep) {
    ...
    const Y = () => {
      if (M.isMounted) {
        ...
        let Re = ye, St;
        Va(M, !1), ye ? (ye.el = Le.el, N(M, ye, me)) : ye = Le, Se && uo(Se), (St = ye.props && ye.props.onVnodeBeforeUpdate) && Cn(St, Ee, ye, Le), Va(M, !0);
        ...
        Va(M, !1), Ee && uo(Ee), !rn && (ye = we && we.onVnodeBeforeMount) && Cn(ye, Re, $), Va(M, !0);
        {
    ...
    if (Le && (Re = me && me.onVnodeBeforeUnmount) && Cn(Re, $, M), ge & 6)
      ...
      Ee && _a(M, null, $, "beforeUnmount"), ge & 64 ? M.type.remove(
        M,
        $,
        j,
        _e,
    ...
    (Le && (Re = me && me.onVnodeUnmounted) || Ee) && Ht(() => {
      Re && Cn(Re, $, M), Ee && _a(M, null, $, "unmounted");
    }, j);
  }, H = (M) => {
...
function Tg(e) {
  ...
  })), m = ct("validation"), y = V(() => e.name ?? Ce(n));
  Bl(() => {
    i.register?.({
      id: y.value,
      vm: m,
...
const Xu = (e) => {
  ...
  setup(e, t) {
    ...
    return te(() => {
      const Z = b.value ? "a" : e.tag, ce = a.title || e.title != null, xe = a.subtitle || e.subtitle != null, _e = !!(e.appendAvatar || e.appendIcon), Ie = !!(_e || a.append), M = !!(e.prependAvatar || e.prependIcon), $ = !!(M || a.prepend);
...
const sc = R({
  ...
  setup(e, t) {
    ...
    function J(X) {
      ...
      function xe() {
        let j = _e();
        return j || x.at(-1) === x.at(-2) && (x = x.slice(0, -1), w++, j = _e(), j) || (w = 0, j = _e(), j) ? j : (x = X.key.toLowerCase(), _e());
      }
      function _e() {
    ...
    return ue(C, () => {
      ...
      return p(Mn, U({
        ...
        modelValue: c.value.map((_e) => _e.props.title).join(", "),
        name: void 0,
        "onUpdate:modelValue": G,
        focused: m.value,
        "onUpdate:focused": (_e) => m.value = _e,
        ...
        "append-inner": function() {
          for (var _e = arguments.length, Ie = new Array(_e), M = 0; M < _e; M++)
            Ie[M] = arguments[M];
...
function Wl(e, t, n, a) {
  const l = /* @__PURE__ */ se([]), i = /* @__PURE__ */ se(/* @__PURE__ */ new Map()), o = V(() => a?.transform ? Ce(t).map((s) => [s, a.transform(s)]) : Ce(t));
  at(() => {
...
function __(e, t, n) {
  ...
  function b(D, L, W, N) {
    const J = t.event, q = c.value(D.input), O = k(D.input), ne = D.start.hour < 12 && D.end.hour >= 12, K = qC(D.start, D.end) <= u.value, H = (ce, xe) => a.getFormatter({
      ...
      if (D.start.hasTime)
        if (W) {
          const xe = G(), _e = K ? ", " : _("br", null, null);
          return _("span", {
            class: "v-event-summary"
          }, [_("strong", null, [ce]), _e, xe]);
        } else {
          const xe = H(D.start);
          return _("span", {
            class: "v-event-summary"
...
const Kh = R({
  ...
  function B(Z) {
    const ce = F.value?.$el;
    if (!ce) return;
    const xe = t.direction === "vertical", _e = xe ? "top" : "left", Ie = xe ? "height" : "width", M = xe ? "clientY" : "clientX", {
      [_e]: $,
      [Ie]: j
  ...
  setup(e, t) {
    ...
    function H(X, fe) {
      ...
      if ([N, J, q, O].includes(X.key)) {
        const _e = (h.value ? [i.value ? N : J, y.value ? q : O] : g.value !== i.value ? [N, O] : [J, O]).includes(X.key) ? 1 : -1, Ie = X.shiftKey ? 2 : X.ctrlKey ? 1 : 0;
        _e === -1 && fe === s.value && !Ie && !Number.isInteger(ce) ? fe = fe - ce % 1 * Z : fe = fe + _e * Z * K.value[Ie];
      } else if (X.key === W)
        fe = r.value;
      else if (X.key === L)
        fe = s.value;
...
const Pc = R({
  ...
  setup(e, t) {
    ...
    return te(() => {
      const ce = Ti.filterProps(e), xe = qa.filterProps(Be(e, ["multiSort"])), _e = Xa.filterProps(e), Ie = Za.filterProps(e);
      return p(Za, U({
        class: ["v-data-table", {
        ...
        }), a)]), a.thead?.(Z.value), !e.hideDefaultBody && _("tbody", null, [a["body.prepend"]?.(Z.value), a.body ? a.body(Z.value) : p(Xa, U(n, _e, {
          items: N.value
  ...
  setup(e, t) {
    ...
    return te(() => {
      ...
      return p(Za, U({
        ...
        }, [_("table", null, [a.colgroup?.(_e.value), !e.hideDefaultHeader && _("thead", {
          key: "thead"
        }, [p(qa, U(Ie, {
          multiSort: !!e.multiSort
        }), a)]), a.thead?.(_e.value), !e.hideDefaultBody && _("tbody", {
          key: "tbody"
        }, [_("tr", {
          ref: O,
          style: {
        ...
        }, null)]), a["body.prepend"]?.(_e.value), p(Xa, U(n, M, {
          items: xe.value
        }), {
          ...a,
          item: (j) => p(uh, {
        ...
        }, null)])]), a.tbody?.(_e.value), a.tfoot?.(_e.value)])]),
        bottom: () => a.bottom?.(_e.value)
      });
    }), {
      calculateVisibleItems: Z,
      scrollToIndex: ce
...
`) : G;
    ...
    return ue(r, (G, X) => {
      ...
      const _e = l.getMonth(xe), Ie = l.getYear(xe);
      _e !== m.value && (m.value = _e, K()), Ie !== h.value && (h.value = Ie, H()), x.value = l.isBefore(ce, xe);
    }), te(() => {
      const G = El.filterProps(e), X = Be(Ks.filterProps(e), ["viewMode"]), fe = Gs.filterProps(e), Z = qs.filterProps(e), ce = Be(Xs.filterProps(e), ["modelValue"]), xe = Be(Zs.filterProps(e), ["modelValue"]), _e = {
        color: v.value,
        header: w.value,
        transition: C.value
      };
...
const TI = R({
  ...
  setup(e, t) {
    ...
    function A(H) {
      ...
      if (new RegExp(`^-?\\d*${To(c.value)}?\\d*$`).test(ce) || (H.preventDefault(), G.value = xe, Pe(() => m.value = xe)), e.precision != null) {
        if (ce.split(c.value)[1]?.length > e.precision) {
          ...
          const _e = (fe ?? 0) + H.data.length;
          G.setSelectionRange(_e, _e);
        }
    ...
    return te(() => {
      ...
      const _e = w.value === "split" ? _("div", {
        class: "v-number-input__control"
      }, [p(tn, {
        vertical: !0
      }, null), fe()]) : e.reverse || w.value === "hidden" ? void 0 : _(he, null, [xe(), ce()]), Ie = n["append-inner"] || _e, M = w.value === "split" ? _("div", {
        class: "v-number-input__control"
      }, [Z(), p(tn, {
        vertical: !0
      ...
      return p(Mn, U({
        ...
        "append-inner": Ie ? function() {
          for (var j = arguments.length, re = new Array(j), z = 0; z < j; z++)
            re[z] = arguments[z];
          return _(he, null, [n["append-inner"]?.(...re), _e]);
        } : void 0,
  ...
  setup(e, t) {
    ...
    function W(O) {
      ...
      const {
        ...
        x: X - H,
        y: K - fe
      }, xe = Math.round(D(Z, ce) - e.rotate + 360) % 360, _e = e.double && B(Z, ce) < (G + G * w.value) / 4, Ie = Math.ceil(15 / y.value);
      let M;
      for (let $ = 0; $ < Ie; $++)
        if (M = F(xe + $ * y.value, _e), g(M) || (M = F(xe - $ * y.value, _e), g(M))) return L(M);
    }
    function N(O) {
...
const lT = {
  __name: "VRangeSlider",
  setup(e) {
    const t = De("children"), n = De("model_value"), a = De("error"), l = De("reverse"), i = De("density"), o = De("max_width"), r = De("min_width"), s = De("width"), u = De("elevation"), c = De("rounded"), d = De("tile"), f = De("theme"), v = De("color"), k = De("name"), m = De("disabled"), y = De("max"), h = De("min"), x = De("step"), w = De("id"), b = De("base_color"), S = De("prepend_icon"), g = De("append_icon"), I = De("readonly"), C = De("ripple"), T = De("label"), P = De("center_affix"), F = De("glow"), A = De("icon_color"), B = De("hide_spin_buttons"), D = De("hint"), L = De("persistent_hint"), W = De("messages"), N = De("direction"), J = De("error_messages"), q = De("max_errors"), O = De("rules"), ne = De("validate_on"), K = De("validation_value"), H = De("focused"), G = De("hide_details"), X = De("thumb_color"), fe = De("thumb_label"), Z = De("thumb_size"), ce = De("show_ticks"), xe = De("ticks"), _e = De("tick_size"), Ie = De("track_color"), M = De("track_fill_color"), $ = De("track_size"), j = De("no_keyboard"), re = De("strict");
    return (z, oe) => {
      const me = Nv("v-range-slider", !0);
      return XS(), QS(me, {
        modelValue: Ce(n),
        error: Ce(a),
        reverse: Ce(l),
        ...
        _: 1
        /* STABLE */
      }, 8, ["modelValue", "error", "reverse", "density", "maxWidth", "minWidth", "width", "elevation", "rounded", "tile", "theme", "color", "name", "disabled", "max", "min", "step", "id", "baseColor", "prependIcon", "appendIcon", "readonly", "ripple", "label", "centerAffix", "glow", "iconColor", "hideSpinButtons", "hint", "persistentHint", "messages", "direction", "errorMessages", "maxErrors", "rules", "validateOn", "validationValue", "focused", "hideDetails", "thumbColor", "thumbLabel", "thumbSize", "showTicks", "ticks", "tickSize", "trackColor", "trackFillColor", "trackSize", "noKeyboard", "strict"]);
    };
  }
