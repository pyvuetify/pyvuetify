...
class DataTable(anywidget.AnyWidget):
    ...
    Args:
        ...
        sort_by: Changes which item property (or properties) should be used for sort order. Can be bound to external variable using **v-model:sortBy**..
...
**Note**: object notation requires at least **v3.11.0**
        ...
        group_by: Configures attributes (and sort order) to group items together. Can be customized further with `group-header` and `group-summary` slots.
...
- **intersection**: There is at least one match from the custom filter, and all columns match the custom key filters.
        ...
        header_props: Pass props to the default header. See [`v-data-table-headers` API](/api/v-data-table-headers) for more information.
        ...
        collapse_icon: Icon to display when the expandable row is expanded.
        expand_icon: Icon to display when the expandable row is collapsed.
        group_collapse_icon: Icon to display when the row group is expanded.
        group_expand_icon: Icon to display when the row group is collapsed.
    ...
    seealso:
        `https://vuetifyjs.com/api/v-data-table`__
    """
